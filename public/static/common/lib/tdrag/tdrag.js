        ...
        Dragfn.prototype = {
                    ...
                    sortBox:function(obj){
                        ...
                        for ( var i = 0; i < arr.length; i++) {
                            for ( var j = i + 1; j < arr.length; j++) {
                                if(Number(arr[i].attr("cuika.view.cuika.index.index")) > Number(arr[j].attr("cuika.view.cuika.index.index"))){
                                    var temp = arr[i];
                                    arr[i] = arr[j];
                                    arr[j] = temp;
                                }
            ...
            move: function (ev, obj) {
               ...
               /* if(self.options.changeWhen=="move") {
                ...
                if(self.options.pos==true){
                    self.animation(obj, self.aPos[$(obj).attr("cuika.view.cuika.index.index")]);
                }
                self.options.cbEnd();
                if(self.options.handle!=null){
            ...
            pack: function(ele,click){
                ...
                if(self.options.pos==false){
                    ...
                    if (self.firstRandom == false) {
                        ...
                        for (var i = 0; i < ele.length; i++) {
                            $(ele[i]).attr("cuika.view.cuika.index.index", sortarr[i]);
                    ...
                    if (self.firstRandom == false) {
                        ...
                        for (var j = 0; j < ele.length; j++) {
                            self.aPos[j] = {
                                left: ele[$(ele).eq(j).attr("cuika.view.cuika.index.index")].offsetLeft,
                                top: ele[$(ele).eq(j).attr("cuika.view.cuika.index.index")].offsetTop
                            };
                        }
                    } else {
                        //第一次
                    ...
                    //第二个循环布局转化
                    for (var i = 0; i < ele.length; i++) {
                        $(ele[i]).attr("cuika.view.cuika.index.index", arr[i]);
            ...
            sort: function(){
                ...
                for ( var i = 0; i < arr_li.length; i++) {
                    for ( var j = i + 1; j < arr_li.length; j++) {
                        if(Number($(arr_li[i]).attr("cuika.view.cuika.index.index")) > Number($(arr_li[j]).attr("cuika.view.cuika.index.index"))){
                            var temp = arr_li[i];
                            arr_li[i] = arr_li[j];
                            arr_li[j] = temp;
                        }
            ...
            pointDrag: function(obj){
                ...
                var oNear=self.findNearest(obj);
                if (oNear) {
                    self.animation(obj,self.aPos[$(oNear).attr("cuika.view.cuika.index.index")]);
                    self.animation(oNear, self.aPos[$(obj).attr("cuika.view.cuika.index.index")]);
                    var tmp;
                    tmp = $(obj).attr("cuika.view.cuika.index.index");
                    $(obj).attr("cuika.view.cuika.index.index", $(oNear).attr("cuika.view.cuika.index.index"));
                    $(oNear).attr("cuika.view.cuika.index.index", tmp);
            ...
            sortDrag: function(obj){
                ...
                var oNear=self.findNearest(obj);
                    if(oNear){
                        if(Number($(oNear).attr("cuika.view.cuika.index.index"))>Number($(obj).attr("cuika.view.cuika.index.index"))){
                            //前换后
                            var obj_tmp=Number($(obj).attr("cuika.view.cuika.index.index"));
                            $(obj).attr("cuika.view.cuika.index.index",Number($(oNear).attr("cuika.view.cuika.index.index"))+1);
                            for (var i = obj_tmp; i < Number($(oNear).attr("cuika.view.cuika.index.index"))+1; i++) {
                                self.animation(arr_li[i],self.aPos[i-1]);
                                self.animation(obj,self.aPos[$(oNear).attr("cuika.view.cuika.index.index")]);
                                $(arr_li[i]).removeClass(self.options.moveClass);
                                $(arr_li[i]).attr("cuika.view.cuika.index.index",Number($(arr_li[i]).attr("cuika.view.cuika.index.index"))-1);
                            }

                        }else if(Number($(obj).attr("cuika.view.cuika.index.index"))>Number($(oNear).attr("cuika.view.cuika.index.index"))){
                            //后换前
                            var obj_tmp=Number($(obj).attr("cuika.view.cuika.index.index"));
                            $(obj).attr("cuika.view.cuika.index.index",$(oNear).attr("cuika.view.cuika.index.index"));
                            for (var i = Number($(oNear).attr("cuika.view.cuika.index.index")); i < obj_tmp; i++) {
                                self.animation(arr_li[i],self.aPos[i+1]);
                                self.animation(obj,self.aPos[Number($(obj).attr("cuika.view.cuika.index.index"))]);
                                $(arr_li[i]).removeClass(self.options.moveClass);
                                $(arr_li[i]).attr("cuika.view.cuika.index.index",Number($(arr_li[i]).attr("cuika.view.cuika.index.index"))+1);
                            }
                        }
