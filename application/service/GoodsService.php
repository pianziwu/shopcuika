...
class GoodsService
{
    ...
    public static function GoodsCategoryAll($params = [])
    {
        ...
        foreach ($data as &$row) {
            ...
            foreach ($row['items'] as &$v) {
                $v['container'] && $v['container'] = json_decode($v['container'], true);
            }
        }
    ...
    {
        $where['is_enable'] = 1;
        $field = 'id,pid,icon,name,vice_name,describe,bg_color,big_images,sort,is_home_recommended,seo_title,seo_keywords,seo_desc,container';
    ...
    public static function GoodsDataHandle($params, $data)
    {
        if (!empty($data)) {
            ...
            foreach ($data as &$v) {
                ...
                // PC底部参数处理
                if (isset($v['web_bottom_params']) && $v['web_bottom_params']) {
                    $v['web_bottom_params'] = json_decode($v['web_bottom_params'], true);
                }
    ...
    {
        $params['id'] = $params['goods_id'] ?? null;
        // 请求参数
        $p = [
            [
                'checked_type' => 'empty',
        ...
        // 开始操作
        $data = ['goods_id' => (int)$params['id'], 'user_id' => $params['user']['id']];
    ...
    public static function GoodsSave($params = [])
    {
        ...
        // 电脑端详情
        $content_detail = self::GetFormGoodsDetailParams($params);
        if ($content_detail['code'] != 0) {
        ...
        // 底部参数内容
        isset($params['web_bottom_params']['left']) && $params['web_bottom_params']['left'] = trim($params['web_bottom_params']['left'], ';');
        isset($params['web_bottom_params']['right']) && $params['web_bottom_params']['right'] = trim($params['web_bottom_params']['right'], ';');
        $web_bottom_params = empty($params['web_bottom_params']) ? '' : json_encode($params['web_bottom_params']);

        // 属性
        $goodsAttr = $params['attribute'];
        ...
        $data = [
            ...
            'web_bottom_params' => $web_bottom_params,
        ];
        ...
        if (isset($goods_id) && $goods_id > 0) {
            ...
            // 属性
            $data = Db::name('goods_attribute_join')->where(['goods_id' => $goods_id])->find();
            if ($data == null) {
                $goodsAttr['goods_id'] = $goods_id;
                $ret = Db::name('goods_attribute_join')->insert($goodsAttr);
            } else {
                $ret = Db::name('goods_attribute_join')->where(['attribute_id' => $data['attribute_id']])->update($goodsAttr);
            }
            ...
            // 电脑详情
            $ret = self::GoodsContentDetailInsert($content_detail['data'], $goods_id);
            if ($ret['code'] != 0) {
                // 回滚事务
                Db::rollback();
                return $ret;
     ...
     * @param    [array]          $params [输入参数]
     */
    private static function GetFormGoodsDetailParams($params = [])
    {
        // 开始处理
        $result = [];
        $name = 'goods_detail_';
        foreach ($params AS $k => $v) {
            if (substr($k, 0, 13) == $name) {
                $key = explode('_', str_replace($name, '', $k));
                if (count($key) == 2) {
                    $result[$key[1]][$key[0]] = $v;
                    if ($key[0] == 'images') {
     ...
     * @return  [array]                    [boolean | msg]
     */
    private static function GoodsContentDetailInsert($data, $goods_id)
    {
        Db::name('GoodsDetail')->where(['goods_id' => $goods_id])->delete();
        if (!empty($data)) {
            foreach (array_values($data) as $k => $v) {
                $temp_content = [
                    'goods_id' => $goods_id,
                    ...
                    'title' => $v['title'],
                    'describe' => $v['describe'],
                    'endescribe' => isset($v['endescribe']) ? $v['endescribe'] : '',
                    'sort' => $k,
                    'add_time' => time(),
                ];
                if (Db::name('GoodsDetail')->insertGetId($temp_content) <= 0) {
                    return DataReturn('电脑详情添加失败', -1);
                }
            }
        }
        return DataReturn('添加成功', 0);
    ...
    public static function GoodsCategoryNodeSon($params = [])
    {
        ...
        // 获取数据
        $field = 'id,pid,icon,name,sort,is_enable,bg_color,big_images,vice_name,describe,is_home_recommended,seo_title,seo_keywords,seo_desc,container';
        ...
        if (!empty($data)) {
            foreach ($data as &$v) {
                ...
                $v = array_merge($v, $v['container'] ? json_decode($v['container'], true) : []);
    ...
    public static function GoodsCategorySave($params = [])
    {
        ...
        // 其它附件
        $data_fields = ['icon', 'big_images', 'container_first_background_image', 'container_second_background_image'];
        $attachment = ResourcesService::AttachmentParams($params, $data_fields);
        if ($attachment['code'] != 0) {
            return $attachment;
        }
        ...
        $container['container_first_background_image'] = $attachment['data']['container_first_background_image'];
        $container['container_second_background_image'] = $attachment['data']['container_second_background_image'];
