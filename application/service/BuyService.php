...
class BuyService
{
        ...
        if(!empty($data))
        {
            foreach($data as &$v)
            {
                ...
                if($goods_base['code'] == 0)
                {
                    ...
                    $v['price'] = sprintf('%.2f',(float) $goods_base['data']['spec_base']['price']);
    ...
    public static function BuyTypeGoodsList($params = [])
    {
        ...
        if($ret['code'] == 0)
        {
            ...
            if (isset($params['site_model'])) {
                $site_model = ($common_site_type == 4) ? ((int) $params['site_model']) : $common_site_type;
            } else {
                $site_model = ($common_site_type == 4) ? (0) : $common_site_type;
            }
    ...
    public static function OrderInsert($params = [])
    {
        ...
        // 销售+自提, 用户自选站点类型
        $site_model = ($common_site_type == 4) ? (isset($params['site_model']) ? (int) $params['site_model'] : 0) : $common_site_type;
        ...
        // 销售型,自提点,销售+自提 地址处理
        $address = [];
        if(in_array($site_model, [0, 2], false))
        {
        ...
        $order = [
            ...
            'status'                => ((int) MyC('common_order_is_booking', 0) == 1) ? 0 : 1,
     ...
     * 用户购物车总数
     * @param  array  $params
     * @return int|string [int|string]             [超过99则返回 99+]
     * @author   Devil
     * @blog    http://1000-x.cn/
     * @version 1.0.0
     * @date    2018-09-29
    ...
    public static function OrderInventoryRollback($params = [])
    {
        ...
        // 是否指定商品和数量
        $appoint_buy_number = empty($params['appoint_buy_number']) ? 0 : (int) $params['appoint_buy_number'];
        $detail_where = ['order_id' => $params['order_id']];
        if(!empty($params['appoint_order_detail_id']))
        {
            $detail_where['id'] = (int) $params['appoint_order_detail_id'];
        }
