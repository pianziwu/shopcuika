...
class AdminPowerService
{
    ...
    public static function PowerSave($params = [])
    {
        ...
        if (empty($params['id'])) {
            ...
            if (Db::name('Power')->where(['id' => (int) $params['id']])->update($data) !== false) {
                // 清除用户权限数据
                self::PowerCacheDelete();
    ...
    public static function PowerDelete($params = [])
    {
        ...
        if (Db::name('Power')->delete((int) $params['id'])) {
            // 清除用户权限数据
            self::PowerCacheDelete();
        ...
        if (!empty($data)) {
            foreach ($data as &$v) {
                ...
                if ($v['id'] == 1) {
                    ...
                    $v['item'] = Db::name('Role')->alias('r')->join(['__ROLE_POWER__' => 'rp'], 'rp.role_id = r.id')->join(['__POWER__' => 'p'], 'rp.power_id = p.id')->where(['r.id' => $v['id']])->field('p.id,p.name')->select();
                }
            }
        }
        return $data;
    ...
    public static function RolePowerEditData($params = [])
    {
        ...
        if (!empty($power)) {
            foreach ($power as &$v) {
                ...
                // 获取子权限
                $item = Db::name('Power')->field($power_field)->where(['pid' => $v['id']])->order('sort')->select();
                if (!empty($item)) {
    ...
    public static function PowerMenuInit()
    {
        ...
        $role_id = isset($admin['role_id']) ? (int) $admin['role_id'] : 0;
        // 读取缓存数据
        ...
        if (($role_id > 0 || $admin_id == 1) && empty($admin_left_menu)) {
            ...
            if ($admin_id == 1 || $role_id == 1) {
                ...
                $admin_left_menu = Db::name('Power')->where(['pid' => 0])->field($field)->order('sort')->select();
            } else {
                $field = 'p.id,p.name,p.control,p.action,p.is_show,p.icon';
                $admin_left_menu = Db::name('Power')->alias('p')->join(['__ROLE_POWER__' => 'rp'], 'p.id=rp.power_id')->where([
                    'rp.role_id' => $role_id, 'p.pid' => 0
                ])->field($field)->order('p.sort')->select();
            }
            // 有数据，则处理子级数据
            if (!empty($admin_left_menu)) {
                foreach ($admin_left_menu as $k => $v) {
                    ...
                    // 获取子权限
                    if ($admin_id == 1 || $role_id == 1) {
                        $items = Db::name('Power')->where(['pid' => $v['id']])->field($field)->order('sort')->select();
                    } else {
                        $items = Db::name('Power')->alias('p')->join(['__ROLE_POWER__' => 'rp'], 'p.id=rp.power_id')->where([
                            'rp.role_id' => $role_id, 'p.pid' => $v['id']
                        ])->field($field)->order('p.sort')->select();
                    }
                    // 权限列表
                    if (!empty($items)) {
