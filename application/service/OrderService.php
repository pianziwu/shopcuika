...
class OrderService
{
    ...
    public static function Pay($params = [])
    {
        ...
        // 获取订单信息
        $where = ['id' => (int) $params['id'], 'user_id' => $params['user']['id']];
        ...
        // 支付方式
        $payment_id = empty($params['payment_id']) ? $order['payment_id'] : (int) $params['payment_id'];
    ...
    public static function OrderCancel($params = [])
    {
        ...
        // 获取订单信息
        $where = ['id' => (int) $params['id'], 'user_id' => $params['user_id'], 'is_delete_time' =>0, 'user_is_delete_time' =>0];
        ...
        if(Db::name('Order')->where($where)->update($upd_data))
        {
            ...
            // 订单状态日志
            $creator = isset($params['creator']) ? (int) $params['creator'] : 0;
