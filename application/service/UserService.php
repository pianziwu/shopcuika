...
class UserService
{
    ...
    public static function UserStatusCheck($field, $value)
    {
        ...
        if (!in_array($user['status'], [0, 1], false)) {
    ...
    public static function UserSave($params = [])
    {
        ...
        $data = [
            ...
            'integral'          => (int)$params['integral'],
            'status'            => (int)$params['status'],
            'alipay_openid'     => $params['alipay_openid'] ?? '',
            'baidu_openid'      => $params['baidu_openid'] ?? '',
            'toutiao_openid'    => $params['toutiao_openid'] ?? '',
            'qq_openid'         => $params['qq_openid'] ?? '',
            'qq_unionid'        => $params['qq_unionid'] ?? '',
            'weixin_openid'     => $params['weixin_openid'] ?? '',
            'weixin_unionid'    => $params['weixin_unionid'] ?? '',
            'weixin_web_openid' => $params['weixin_web_openid'] ?? '',
    ...
    public static function UserAddressList($params = [])
    {
        ...
        if (!empty($data)) {
            ...
            $is_default_handle = isset($params['is_default_handle']) ? (int)$params['is_default_handle'] : 1;
    ...
    public static function UserAddressSave($params = [])
    {
        ...
        if (!empty($params['lng'])) {
            $data['lng'] = (float) $params['lng'];
        }
        if (!empty($params['lat'])) {
            $data['lat'] = (float) $params['lat'];
        }
    ...
    public static function Reg($params = [])
    {
        ...
        if($params['type'] == 'sms')
        {
            $obj = new Sms($verify_params);
        } else if($params['type'] == 'email') {
            $obj = new Email($verify_params);
