...
class PaymentService
{
    ...
    public static function PaymentList($params = [])
    {
        ...
        {
            $where['is_open_user'] = (int) $params['is_open_user'];
        }
    ...
    public static function PaymentUpdate($params = [])
    {
        ...
        $data = [
            ...
            'is_open_user'      => isset($params['is_open_user']) ? (int) $params['is_open_user'] : 0,
        ];

        $data['upd_time'] = time();
        if(Db::name('Payment')->where(['id'=> (int) $params['id']])->update($data))
        {
