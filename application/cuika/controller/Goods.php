...
class Goods extends Common
{
    ...
    public function favorList()
    {
        ...
        // 分页
        $number = 10;
        $page = max(1, isset($this->data_post['page']) ? (int) $this->data_post['page'] : 1);
        ...
        $this->assign('good_list',$data['data']);
        $this->assign('paginate',$result);
        if (count($data['data']) > 0){
            return $this->fetch('shoucang');
        }
        return $this->fetch('shoucang-no-goods');
    }
