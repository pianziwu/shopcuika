...
function AsciiToStr($ascii)
{
    ...
    if(!empty($ascii))
    {
        ...
        foreach ($asc_arr as $iValue) {
            $str .= chr(hexdec($iValue[1].$iValue[0]));
        }
        // 编码处理
        $encode = mb_detect_encoding($str);
        if($encode != 'UTF-8')
 ...
 * @return null  [mixed]                    [配置信息值,没找到返回null]
 * @author   Devil
 * @blog     http://1000-x.cn/
 * @version  0.0.1
 ...
 * @return bool|false|int [boolean]     [存在true, 则false]
 * @author   Devil
 * @blog     http://1000-x.cn/
 * @version  0.0.1
 * @datetime 2016-12-03T21:58:54+0800
...
{
    $code = '';
    for($i=0, $iMax = (int) $length; $i< $iMax; $i++) {
        $code .= mt_rand(0, 9);
    }
    return $code;
}
